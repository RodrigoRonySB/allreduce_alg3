...
int main(int argc,char**argv){
    ...
    FILE* fp=NULL; if(r==0){ fp=fopen(outcsv,"w"); if(!fp){perror("fopen"); MPI_Abort(MPI_COMM_WORLD,1);} fprintf(fp,"datasize_bytes,m_elems,best_us_alg3,best_us_native\n"); }
    ...
    for(int i=0;i<ns;++i){
        ...
        if(r==0){
            ...
            fprintf(fp,"%.0f,%d,%.6f,%.6f\n", bytes, m, t_alg3, t_nat);
            fflush(fp);
            printf("m=%d -> alg3=%.2fus native=%.2fus\n", m, t_alg3, t_nat);
        }
    }
    if(r==0) fclose(fp);
    free(sizes);
